...
/**
 *
 * @class		like_bitracker_categories_composite
 * @brief		Categories like class (bitracker application)
 */
class like_bitracker_categories_composite extends classes_like_composite
{
	/**
	...
	public function getMeta( $relId, $selectType=null )
	{
		...
		while( $row = $this->DB->fetch() )
		{
			...
			{
				$return[ $row['cid'] ]['like.url'] = $this->registry->output->buildSEOUrl( "app=bitracker&amp;showcat=" . $row['cid'], "public", $row['cname_furl'], "bitshowcat" );
			}
			...
			if ( $selectType === null OR ( is_array( $selectType ) AND in_array( 'parentTitle', $selectType ) ) )
			{
				$return[ $row['file_id'] ]['like.parentUrl'] = $this->registry->output->buildSEOUrl( "app=bitracker", "public", "false", "app=bitracker" );
			} 
