 ...
 */
class admin_bitracker_tools_maintenance extends ipsCommand
{
	/**
	...
	public function doExecute( ipsRegistry $registry ) 
	{
		...
		$this->html = $this->registry->output->loadTemplate( 'cp_skin_maintenance' );
		
		//-----------------------------------------
		// Set up stuff
		//-----------------------------------------
		
		$this->form_code	= $this->html->form_code	= 'module=tools&amp;section=maintenance';
		$this->form_code_js	= $this->html->form_code_js	= 'module=tools&section=maintenance';
		...
		{	
			...
			break;
			//-----------------------------------------
			case 'recount_peerstats':
				$this->registry->getClass('class_permissions')->checkPermissionAutoMsg( 'bit_tools_rebuild' );
				$this->_recountpeerStats();
			break;
			//-----------------------------------------
			case 'thumbs':
	 ...
	 * @return	@e void
	 */
	protected function _recountpeerStats()
	{
		if( $this->settings['bit_logallbitracker'] == 0 )
		{
			$this->registry->output->global_message = $this->lang->words['t_logall'];
		...
		$this->DB->build( array( 'select'	=> 'count(seeder) as cnt, torrent, seeder',
								 'from'		=> 'bitracker_torrent_peers',
								 'group'	=> 'torrent, seeder'
							)		);
		$outer = $this->DB->execute();
		
		while( $s = $this->DB->fetch($outer) )
		{
			if( !$s['torrent'] )
			{
				continue;
            ...
            $default_nums = array('yes' =>'0', 'no' =>'0');
           			
			$s_ids[$s['torrent']][$s['seeder']] = $s['cnt'];
            

            foreach ($s_ids as $sid => $value)
            {
               $list[] = $sid;
			   $s_ids[$s['torrent']] = array_merge($default_nums, $value);
            }

            $_ids = array_unique($list);
		
			$this->DB->update( "bitracker_torrent_data", array( 'torrent_seeders' => $s_ids[$s['torrent']]['yes'], 'torrent_leechers' => $s_ids[$s['torrent']]['no'] ), "torrent_id=" . $s['torrent'] );

		}
	
		if( count($s_ids) )
		{
			$this->DB->update( "bitracker_torrent_data", array( 'torrent_seeders' => 0, 'torrent_leechers' => 0 ), "torrent_id NOT IN(" . implode( ",", $_ids ) . ")" );
		}
		
		$this->registry->getClass('adminFunctions')->saveAdminLog('All peers stats have been recounted');
		$this->registry->output->global_message = 'All peer stats recounted';
		$this->_mainScreen();
	}	
