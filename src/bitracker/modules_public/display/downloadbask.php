...
class public_bitracker_display_download extends ipsCommand
{
	...
	public function doExecute( ipsRegistry $registry )
	{
		...
		// @link	http://bugs.---.com/tracker/issue-24706-tracking-login-redirect/
		//-----------------------------------------
	...
	protected function _displayConfirm()
	{
		...
		$this->output .= $this->registry->getClass('output')->getTemplate('bitracker_other')->confirmdownload( $info, $files, $category );
	}
	...
	protected function _doVersiondownload()
	{
		...
		if( !$record AND ( $category['cdisclaimer'] or count($records) > 1 ) )
		{
			...
			$this->output .= $this->registry->getClass('output')->getTemplate('bitracker_other')->confirmVersiondownload( $info, $files, $category );
			return;
		}
		else
		{
	...
	protected function _downloadLocalFile( $file=array() )
    {
	    ...
	    // @link	http://bugs.---.com/tracker/issue-32155-ie8-https
	    //------------------------------------------
