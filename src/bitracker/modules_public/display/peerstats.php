...
class public_bitracker_display_peerstats extends ipsCommand
{
	/**
	...
	public function doExecute( ipsRegistry $registry )
	{
		...
		//---------------------------------------------
		// Get all the current peers for this torrent?
		//---------------------------------------------
         ...
         //---------------------------------------
          ...
          if( $peers[$peer]['seeder'] == yes && $peers[$peer]['downloaded'] = 0 )
           {
               $peers[$peer]['f_ratio'] = $peers[$peer]['uploaded'] / $file['torrent_filesize'];
           }

            //$peers[$peer]['f_ratio'] = (($peers[$peer]['downloaded'] > 0) ? ($peers[$peer]['uploaded'] / $peers[$peer]['downloaded']) : 1);
            $peers[$peer]['f_ratio'] = $peers[$peer]['uploaded'] / $peers[$peer]['downloaded'];
		...
		$this->output .= $this->registry->getClass('output')->getTemplate('bitracker_file')->torrentPeersactiv($peers);
