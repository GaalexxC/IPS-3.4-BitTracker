...
class public_bitracker_post_submit extends ipsCommand
{
	...
	protected function _continueForm( $type='new' )
	{
		...
		else
		{
			$file = array( 	'code' 				=> 'add_comp',
							...
							'file_desc'			=> $_POST['Post1'],
							'file_tech'			=> $_POST['Post2'],
							'file_extra'		=> $_POST['Post3'],
		...
		$file['file_desc']		= $_POST['Post1'] ? $_POST['Post1'] : $file['file_desc'];
		$file['file_tech']		= $_POST['Post2'] ? $_POST['Post2'] : $file['file_tech'];
		$file['file_extra']		= $_POST['Post3'] ? $_POST['Post3'] : $file['file_extra'];
		...
		$file['_editor'] = $editor->show( 'Post1', array(), $file['file_desc'] );
		$file['_editor1'] = $editor->show( 'Post2', array(), $file['file_tech'] );
		$file['_editor2'] = $editor->show( 'Post3', array(), $file['file_extra'] );
	...
	protected function _mainSave( $type='new' )
	{
		...
		$_POST['Post1'] 	= IPSText::stripslashes( $_POST['Post1'] );
		$_POST['Post2'] 	= IPSText::stripslashes( $_POST['Post2'] );
		$_POST['Post3'] 	= IPSText::stripslashes( $_POST['Post3'] );
		$catid			= intval($this->request['file_cat']);
		$file			= array();
		...
		$file['file_desc']		= trim( IPSText::stripslashes( $_POST['Post1'] ) );
		$file['file_tech']		= trim( IPSText::stripslashes( $_POST['Post2'] ) );
		$file['file_extra']		= trim( IPSText::stripslashes( $_POST['Post3'] ) );
		...
		if( IPSText::mbstrlen( $file['file_tech'] ) < 1 )
		{
			$this->error_message = $this->lang->words['addtorrent_error_torrenttech'];
			$this->_continueForm( $type );
			return;
		}

		if( IPSText::mbstrlen( $file['file_extra'] ) < 1 )
		{
			$this->error_message = $this->lang->words['addtorrent_error_torrentextra'];
			$this->_continueForm( $type );
			return;
		}
		...
		$file['file_desc'] = $editor->process( $_POST['Post1'] );
		$file['file_tech'] = $editor->process( $_POST['Post2'] );
		$file['file_extra'] = $editor->process( $_POST['Post3'] );
		...
		$file['file_tech'] = IPSText::getTextClass('bbcode')->preDbParse( $file['file_tech'] );
		...
		{
			$_test	= IPSText::getTextClass('bbcode')->preDisplayParse( $file['file_tech'] );
		...
		$file['file_extra'] = IPSText::getTextClass('bbcode')->preDbParse( $file['file_extra'] );
		...
		{
			$_test	= IPSText::getTextClass('bbcode')->preDisplayParse( $file['file_extra'] );
		...
		//----------------------------------------------  
         ...
         if ( $this->settings['use_friendly_urls'] AND $this->settings['seo_r_on'] )
            {
              $default_announce = $this->settings['board_url'] . '/announce';
            }
             else  
            {
              $default_announce = $this->settings['board_url'] . '/index.php?app=bitracker&amp;module=client&amp;section=announce';
            }
        ...
        $file['torrent_infohash'] = strtoupper(trim($this->TorrentInfo['torrent_infohash']));
        $file['torrent_filesize'] = intval($this->TorrentInfo['torrent_filesize']);
		...
		if( $type == 'edit' )
		{
			...
			$this->DB->delete( "bitracker_files_records", 'record_file_id=' . $file['file_id'] . ' AND record_backup=0' );
 			...
 			$this->DB->delete( "bitracker_torrent_data", 'torrent_id=' . $file['file_id']  );           
		}
